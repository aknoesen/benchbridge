...
def main():
    ...
    write_app_catalog(catalog)
    if skipped:
        print(f"SKIPPED {len(skipped)}: {', '.join(skipped)}")


def svg_inner(svg_text):
    """Strip the xml header / generator comment / outer <svg> tag, keeping the
    drawable content (coordinates stay in viewBox space)."""
    s = re.sub(r"<\?xml[^>]*\?>\s*|<!--.*?-->\s*", "", svg_text, flags=re.S)
    s = re.sub(r"^\s*<svg[^>]*>", "", s, count=1)
    s = s.rsplit("</svg>", 1)[0]
    return s.strip()


def write_app_catalog(catalog):
    """Emit the catalog as a generated TypeScript module for the app (SCH-11 P3).
    Ink colours (#000 lines + #222222 terminal dots) become `currentColor` so the
    editor can tint a symbol (selection highlight) by setting CSS `color`."""
    app_path = os.path.normpath(os.path.join(HERE, "..", "src", "core", "symbolCatalog.ts"))
    entries = {}
    for sid, sym in catalog.items():
        inner = svg_inner(sym["svg"])
        inner = re.sub(r"(['\"])#(?:000|222222)\1", r"\1currentColor\1", inner)
        entries[sid] = {"viewBox": sym["viewBox"], "pins": sym["pins"], "inner": inner}
    with open(app_path, "w", encoding="utf-8") as f:
        f.write(
            "// GENERATED by prototype/build.py (SCH-11) — DO NOT EDIT BY HAND.\n"
            "// circuitikz-rendered symbol catalog: per symbol the drawable SVG inner\n"
            "// content (coords in viewBox space, ink = currentColor) plus the terminal\n"
            "// anchor coordinates extracted from the render (see prototype/README.md).\n\n"
            "export interface CatalogPin { id: string; x: number; y: number }\n\n"
            "export interface CatalogSymbol {\n"
            "  viewBox: string\n"
            "  pins: CatalogPin[]\n"
            "  inner: string\n"
            "}\n\n"
            "export const SYMBOL_CATALOG: Record<string, CatalogSymbol> = "
            + json.dumps(entries, indent=1)
            + "\n"
        )
    print(f"Wrote {len(entries)} symbols -> {app_path}")
