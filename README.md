...
- **Breadboard** — transfer a drawn schematic to a parametric solderless board (2-pin parts, transistors in a **TO-92** footprint, op-amps as their real DIP package (**8-pin single** for OP27/37/97, **14-pin quad** for OP482/484, the **TLV9062** on a SOIC-8-to-DIP adapter), and the INA125 as a **16-pin DIP**), run jumpers, and **Check** that the board is electrically equivalent to the schematic, with per-connection feedback (including the INA125's required reference/sense/sleep straps). The board provides fixed M2K connector strips with pre-wired, colour-coded power rails (V+ red, V− blue, GND neutral), and jumpers on a supply net take that rail's colour. Resistor legs enforce a realistic minimum hole spacing. Practice mode colours nets live; Bench mode hides them until Check. The combined view has a draggable splitter and a stacked / side-by-side toggle. Save/Open a "lab" bundle (schematic + board + generator settings) as one file.
...
- Getting started: a **Flashlight** (supply → resistor → LED) and a bare **signal** (W1 → scope) to learn the bench before any theory.
...
- Transimpedance (current → voltage): a single-supply **photodiode TIA** on the **TLV9062** (light drives the output up from a small reference; open the Network Analyzer's Transimpedance mode to see |Z| flat at R_f then roll off), and an **AC transimpedance amp** on the OP484.
