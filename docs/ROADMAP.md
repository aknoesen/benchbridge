...
| F-4 | **Per-part op-amp board packages** (promoted from stretch — real default-path bug). The board hardcodes the LMC662 8-pin DIP for every op-amp, so the default **OP484 (14-pin quad)** shows a wrong "LMC662" 8-pin part. Step 0: **remove ADTL082 + AD8542** (breakout boards, not breadboard DIPs — andre 2026-06-28), so every remaining kit op-amp is a DIP. Then drive the footprint from `c.part` + the `opamps.ts` catalog: 8-DIP single (OP27/37/97), 14-DIP quad (OP482/484), 8-DIP dual (off-kit LMC662); keep INA125 16-DIP. Optional: "show one valid layout" hint. Board/UI only, no `core/signal.ts`. See `docs/specs/f4-opamp-board-packages.md`. | F-3 | TODO |
