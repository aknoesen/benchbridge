...
| SCH-13 | **Symbols reflect part state (use the already-rendered SCH-11 glyphs) — andre, 2026-07-06.** The Tikz→SVG catalog rendered several glyphs we never wired up. Map the meaningful ones in `symbolFor` (`core/symbolArt.ts`): **W1/W2/`vsource` show the actual wave shape** (`vsource_sin`/`_square`/`_tri`/`_saw` by `waveType`; pins/nets/ground-return/sim unchanged), and the **SCH-10 polarized electrolytic uses `polarized_cap`** (plain ceramic stays `capacitor`). `current_source`/`dc_source`/`njfet`/`pjfet` stay unused (no matching part). Visual only, no `core/signal.ts`. See `docs/specs/sch13-symbol-state.md`. | SCH-11 | **DONE** |
| SCH-14 | **Parts must not rotate/drag off-canvas and get lost (BUG, data loss — andre, 2026-07-06).** The canvas SVG is 100%/no-scroll, and `rotateComponentWithWires` has no bounds check while drag clamps only the lower bound, so a part swung/pushed past the edge is invisible and unreachable. Clamp every move/rotate so the whole part (body + all terminals via `terminalsOf`) stays in `[0..maxGx]×[0..maxGy]` (shift-inward or refuse the rotate), plus a one-time clamp-into-view for out-of-bounds parts on load. Editor + `core/schematic.ts` + tests, no `core/signal.ts`. See `docs/specs/schematic-rearrange.md`. | SCH-1 | **DONE** |
| SCH-15 | **Rearranging without delete-and-rewire â reliable rubber-banding (BIGGER â andre, 2026-07-06).** "One cannot move connections; one has to delete and rewire." A move-with-wires path exists (`moveComponentWithWires`/`moveSelectionBy`/`bridgeWiresForMove`) but connections don't reliably follow. Investigate-then-fix: moving a part/selection keeps every connection (touch-connections â wires) and **re-routes attached wires orthogonally** (`orthoRoute`) instead of stretching to diagonals; add wire-segment/endpoint drag to re-route a connection without deleting it; a discoverable move affordance; one undo per gesture. Editor + `core/schematic.ts` + tests, no `core/signal.ts`, Check/sim unchanged. See `docs/specs/schematic-rearrange.md`. | SCH-14 | IN PROGRESS (core DONE; wire-drag gesture → follow-up) |
| SCH-11 | **Schematic symbols → circuitikz textbook catalog + instruments (video-driven, andre 2026-07-02).** In a demo the breadboard's realistic parts pop while the schematic reads flat. Reproduce the **circuitikz ANSI symbol catalog** as SVG (enumerate from the CTAN manual + installed package, batch-render each via `dvisvgm`/`pdf2svg`) and map the app's part kinds onto it, matching the circuitikz style of **andre's own course reader** (max pedagogical consistency); include the **signal-generator, oscilloscope, and power-supply** symbols (circuitikz has the signal/scope elements andre uses). **Visual layer only:** preserve terminal anchor points, wire-attach, hit targets, drag/rotate, PNG export; **no `core/signal.ts`** (canary untouched); decoupled from the netlist. **Boundary:** do NOT pull in WIRE-4's always-present-terminal refactor. Develop on branch **`sch11-ansi-symbols`**; merge to main only if small + low-risk (292/292 incl. canary, build clean, no signal.ts, export + editor interactions intact) before survey **wave 1**. See `docs/specs/sch11-schematic-symbols.md`. *Delivered (merged 2026-07-03, andre's acceptance):* 28-symbol circuitikz catalog rendered via latex→DVI→dvisvgm with pin-anchor extraction, app render swap on a white/green-pad/blueprint paper canvas (PNG export always white), model-space flip (F) + hover-targeted R/F, pin-magnetic modeless wiring, ghost-place + right-click menu + hover hint, M2K port instrument badges, option-B CH1/CH2 measurement palette with scope↔voltmeter view, and the **two-terminal instruments** rework (scope CH1/CH2 absorb 1−/2−; W1/W2 draw their bonded ground return; unwired − = single-ended) proven **sim-result-equivalent** across the example library (`sch11-sim-equivalence.test.ts` + committed baseline). The broken `ina125-amp` example was removed (part stays placeable); legacy saves migrate via `migrateSchematic`. | SCH-1 | DONE |
| FIT-1 | **The schematic is always fully framed on the sketch pad (BUG — andre, 2026-07-13).** The editor drew at a fixed 1:1 pixel view, so a part's glyph/label overhanging its grid node rendered past the pad edge — on the RC low-pass the left-most instrument (CH2 + its ground) was cut off, unreadable and unreachable. Full auto-fit (andre's call, over fit-on-load-only): a pure `fitToContent` (`core/viewport.ts`) computes the pan+zoom from the **rendered** extent (SVG `getBBox` over a content layer — glyphs, ground symbols and value labels included; grid coordinates alone would still clip them), re-framed on load/place/move/rotate/flip/delete/wire and on pad resize, margin on all four sides, zoom clamped, no jitter (the fit measures pre-transform, so it can't feed back). **Also removes SCH-14's mount/open-time `clampAllInBounds`**, which rewrote the drawing to fit the mounting pane and in the short stacked Board pane *sheared* it — every example lost nets (rc-lp 6→4, summing 9→4), silently changing the circuit and autosaving it. Keeping the drawing reachable is the viewport's job now; the interactive drag/rotate clamps stay (and clamp to the framed region, so they're pane-size independent). Editor + new `core/viewport.ts` + tests, no `core/signal.ts`. See `docs/specs/fit1-schematic-autofit.md`. | SCH-14 | **DONE** |
| DRAG-1 | **A multi-step drag severed connections (BUG, data loss — andre, 2026-07-13).** Dragging a part off a touch-connection (e.g. the ground sitting on the scope −) cut it loose and left a stranded one-cell stub. The editor applied a drag *incrementally* (every mousemove against the current state); `moveComponentWithWires` bridges a touch-connection into a real wire, but that wire isn't in `attached` (captured at mousedown), so the bridge was built on the first mousemove and then left behind. A one-move drag worked — which is why it was never caught. Fix: apply every move from `dragBase` (the schematic at mousedown) with the TOTAL delta, making the gesture idempotent; group drag switches to the grab point. 4 tests replay the editor's loop, one pinning the old behaviour as the regression. | SCH-15 | **DONE** |
| SCH-16 | **Re-route a wire without deleting it (SCH-15's deferred remainder — andre, 2026-07-13: "wires are stuck").** Drag a wire's ENDPOINT to re-attach it (magnetic snap to pins, `moveWireEnd`) or its BODY to slide the whole run (`moveWireBy`, which bridges any end that sat on a pin so the connection survives). A press on a wire arms a drag but only commits on the first move, so a press that never moves is still a click (select, or start a pin-wire when on a pin). Grab handles on the hovered/selected wire. Pure core ops + 6 tests. **Drag gestures are unverified in the UI** — the available browser automation fires no intermediate mousemove events, so no drag can be driven; needs a human pass. | SCH-15, DRAG-1 | **DONE** |
| SCH-16b | **Polyline wire model — the "strange shapes" fix (andre, 2026-07-13).** SCH-16's first cut grew an orthogonal bridge at each end on EVERY drag, so a diagonal drag added two L-bends (moved wire + two Ls + the segment left behind = a **closed rectangle**) and each gesture added more — wire soup. Replaced `moveWireBy` with `dragWireSegment` on three rules: a segment slides **perpendicular to itself** (so a connector is one straight segment, never an L, never a loop); a neighbour at a bare corner **stretches** to follow (re-pointed, not duplicated); only a **pin-anchored** end grows a connector, exactly one, and the next drag stretches *that* one — nothing accumulates. Also: a wire end sitting ON a component pin was un-grabbable (the terminal dot paints over it), so the re-route gesture was unreachable — `onComponentDown` now arms an endpoint drag alongside the wiring gesture (press = wire, press-and-drag = pull the end off the pin). | SCH-16 | **DONE** |
| FIT-2 | **Frame on LOAD only — editing must not move the view (andre, 2026-07-13).** FIT-1 re-framed on every change to the drawing, so placing a part re-centred the canvas under the cursor ("place one component - it snaps to the center - NO"). The refit is now keyed on an explicit `fitKey` (mount / Example / Open / Clear) + the pad size; `sch` is deliberately not a dependency. Framing is about *arriving* at a drawing you can see whole. | FIT-1 | **DONE** |
| FIT-3 | **A pad resize only re-frames if the drawing no longer fits (andre, 2026-07-13).** FIT-2 was insufficient: the ResizeObserver still re-framed on ANY resize, and placing a part selects it → the Selected inspector appears → the side panel reflows → the pad resizes → the drawing re-centres. So *every* placement centred it. Reproduces only in a window where that reflow changes the panel width (mine never did — andre's did every time, which is why it was twice reported "fixed" when it was not). A resize now re-frames only when the content is no longer fully visible; a load still frames unconditionally. | FIT-2 | **DONE** |
| PLACE-1 | **One-shot placement — the tool does not stay armed (andre, 2026-07-13).** "The program goes to a second resistor automatically — NO. Each component that one uses should be specifically selected — do not assume what I want to do next." Placing a part now drops the tool back to Select, clears the ghost, resets the place angle/mirror, and leaves the new part selected — so `R`/`F` act on the part you just placed instead of the ghost (which is what made standing a resistor upright such a fight). | SCH-1 | **DONE** |
| WIRE-1 | **Connection-first schematic — wires stop being editable geometry (andre, 2026-07-13: "I find polyline awkward — what other options are there?").** Declare a connection (click pin A → pin B); the app draws the route and re-draws it whenever anything moves. Nothing to grab, slide, tear or loop. Rationale: **nothing downstream uses wire geometry** — `toCircuit`/netlist/ngspice and the whole board Check consume *nets* — so hand-drawing wires is work the tool demands and then throws away, and it is where every bug of 2026-07-13 lived. Blast radius stays small because every consumer reaches connectivity through one function (`computeNets(s): Map<"gx,gy", net>`): keep the signature, derive it from explicit `connections` instead of coordinates, and `toCircuit`/`netlist.ts`/`breadboard.ts`/the Check are untouched. **Settled:** coordinate coincidence no longer connects (enforces the completeness corollary — no inferred connections). **Staged and reversible until stage 3; stage 2 (the auto-routed examples) is andre's GO/NO-GO.** Hard gate: the sim-equivalence baseline stays BYTE-IDENTICAL. **Supersedes SCH-16/16b** (today's polyline wire editing), which is deleted in stage 3. See `docs/specs/wire-1-connection-first.md`. | SCH-16b | TODO |
...
| INST-1 | **Enforce the M2K instrument model — PRIORITY BUG (andre, 2026-07-06).** Three facets of one model (see the authoritative record `docs/reference/m2k-instrument-model.md`): **(A)** stop auto-drawing a ground on an unwired scope − — the schematic follows the designer, so − wired = differential / − open = single-ended, and the app draws only what the designer wired (removes the `a752c72` scope-neg stub; **must NOT touch** the W1/W2 return grounds or V+/V− reference grounds, which are fixed internal bonds and stay); **(B)** M2K I/O are **singletons** — at most one each of CH1/CH2/W1/W2/V+/V− (GND repeatable), palette greys the placed ones; **(C)** scope ⇄ voltmeter are **mutually exclusive per channel** (one ADC per channel; the panels follow each channel's `view`). Pure editor + panels + tests, **no `core/signal.ts`**, sim/baseline unchanged. Spec `docs/specs/inst1-instrument-model.md` (C shipped as INST-2). | WIRE-1 | **DONE** (A+B `69f50a2`; C/INST-2) |
| EDIT-1 | Rubber-band wires: endpoints follow a component when it is moved/rotated | SCH-1 | DONE |
| WIRE-4 | **Always-present fixed M2K terminals on the schematic (schematic↔board symmetry) — POST-BETA architectural cleanup (andre, 2026-07-02).** Show the full fixed M2K I/O — **W1/W2, 1+/1−/2+/2−, GND, V+/V−** — on **every** schematic, present even when unused, exactly like the board's fixed connector strips (F-5). Non-deletable anchors you wire your circuit into. Payoff: schematic↔board become 1:1 (simpler transfer/Check); examples drop the W1/scope/ground boilerplate and just wire to what's there; the "neutral single-signal" case is trivial (W1→1+ = one trace); GND always present kills a class of floating-input/missing-ground bugs; students always see the real bench I/O. **Folds in two consequences (andre, 2026-07-02):** (a) the schematic **terminal picker goes away** — the W1/W2 · 1±/2± · GND/V± palette items are dead weight once the terminals are always present; (b) a **parts-bin board flow** — a component, as soon as it's picked or on the schematic, **auto-appears on the board in a staging bin** as a **draggable** part you drag-and-drop into place, replacing the click-chip→click-two-holes "Place from schematic" (builds on the ARB-5 drag engine → a real Fritzing-style grab-and-place). **Why POST-BETA:** big refactor — schematic model + editor + `toCircuit`/`netlist` + the transfer + **migrate every example** + **re-verify the 12-bit canary** (brushes the signal path: unconnected terminals must still = "no circuit" so the raw-generator/canary path holds). Large regression surface; do it as a clean, fully re-verified refactor after the beta build is locked, not before. Near-term the neutral-signal example covers the Quickstart need — it uses a **1 MΩ dummy load** to keep the single-signal sim well-posed (not a floating node); **WIRE-4 makes the literal bare-wire `W1 → 1+` version work, dropping the dummy R** (answers CC's signal-sine note: yes). | WIRE-1, F-5 | TODO |
...
| CHECK-1 | **Board Check must not flag an explicit scope-−→GND wire as an error (BUG — andre, 2026-07-13).** On a correctly-built single-ended measurement (scope − explicitly wired to GND, board ties them too) Check reported a phantom short: *"GND and 1- are different nodes, but your board connects them."* Root cause: the Check's schematic side (`schematicExpectation`) read raw `computeNets` names, while `toCircuit` folds every ground symbol **and** the W1/W2 returns into node 0 — and ground symbols are repeatable, so one node read as three. Fix: one shared `groundNetsOf` (`core/schematic.ts`), used by `toCircuit`, `schematicExpectation` and `boardNodeMap`, so the sim and the Check partition the schematic identically. Floating-minus detection (Rule 2 / completeness corollary) is a separate rule and stays intact; no auto-grounding re-introduced. `core/` + tests only, no `core/signal.ts`. See `docs/specs/check1-scope-minus-gnd.md`. | F-2, F-5 | **DONE** |
...
| QS-5 | **"Explore mode" — a toggleable in-app help/learn overlay (andre, 2026-07-02).** Consolidates several ideas into ONE feature: a **toggle** that, when on, adds **hover help on every part + M2K terminal** (what it is — "W1 = signal-gen output," "1+ = scope CH1+"; for a part: its value + "click to select, then change it in the menu"), **plus per-example "things to try" prompts** telling a first-timer what to explore ("select R1, change its value, watch the output move"; "hover W1"), and optional callouts on key terminals. **Scoped so it isn't annoying: default ON in Quickstart-loaded examples, OFF in the normal editor**, and user-toggleable. Teaches *what things are*, *how to interact* (select + change value via the menus), and *what to explore*. Plain hover-tooltips are a small near-term slice; the toggle + per-example prompt layer is the designed piece and belongs with the Quickstart. Board/UI only, no `core/signal.ts`. | QS-4 | TODO |
| QS-4 | **Quickstart redesign** (`docs/specs/quickstart-redesign.md`) — paginated, orientation-first, course-neutral: a flat chapter menu drives one page at a time (no long scroll), Next/Back walk the spine, menu doubles as progress (visited ✓, ~time chips). Chapter order andre-locked (reader-aligned arc): orientation → bench map → **flashlight opener** (LED + ARB-2 board glow, PWM dimming) → voltage-divider quick win (single-ended-vs-differential folded in) → signal→Spectrum+digitization → RC in time → RC in frequency ("same circuit, two views") → **op-amps** (inv-amp + clipping) → I-V curves (Zener XY + Curve Tracer families) → build capstone ending on Breadboard → Check → where-next. New orientation screen (one big idea + signal-flow visual + tour/jump branch buttons, neutral simulation-honest voice); QS-1/QS-2 step content + SVGs reused. | QS-2 | DONE |
| CALC-1 | **In-app EE calculator (POST-BETA — andre, 2026-07-02).** A small calculator utility for the common formulas: **Ohm's law** (V/I/R), **power** (P=VI / I²R / V²/R), **RC/RL τ** (RC, L/R) and **cutoff** f_c (1/2πRC, R/2πL), **RLC resonance** (1/2π√(LC)), **reactance** (X_C, X_L), **voltage divider**, **LED series resistor** ((V_s−V_f)/I), **dB** (20·log). Teaching touch: **show the formula + the substituted values, not just the answer.** Distinctive enhancement: **circuit-aware — pull R/C/L from the drawn circuit** ("your RC: τ = …, f_c = …"), which pairs with the active-learning lessons (flashlight I=V/R, RC page's τ/cutoff) and beats a generic phone calculator. Not beta-critical (lessons work with the student calculating on their own) → **post-beta batch.** Board/UI only, no `core/signal.ts`. | — | TODO |
...
From P.Z.'s review of the deployed app (positive overall — intuitive, Quickstart clear, transfer
...
| GLOW-1 | **Animated / instantaneous LED glow — the "wow" enhancer (POST-BETA / batch 2 — andre, 2026-07-02).** ARB-2's glow is driven by the LED's **average** current, so it's a steady brightness — a triangle/PWM drive settles to one glow level rather than **breathing/pulsing**, and a duty-cycle change is only visible if you can see the board while changing it. Make the glow follow the **instantaneous** current (bind to the live `.tran` waveform, not the average) so a triangle drive makes the LED **breathe** and a PWM duty sweep visibly **dims/brightens** live. High wow, but a rendering feature → not in the beta (the flashlight opener uses the simple **steady** glow, which is already striking on the photoreal board and rock-solid). Board/UI only, no `core/signal.ts`. | ARB-2 | TODO |
...
| ARB-7 | **Symmetric passives are orientation-agnostic in the board Check + auto-router (BUG — andre, 2026-07-06).** `checkEquivalence`/`autoRouteJumpers` hard-bind each 2-leg part's physical legs to fixed schematic terminals (`aHole→A`, `bHole→B`) for **all** `PLACEABLE_KINDS`. Correct for polarized parts (diode/LED/zener/photodiode — a reversed one must fail) but **wrong for symmetric R/C/L**: terminal A/B is an arbitrary label, so a valid board that orients a resistor the other way trips a false *"different nodes, but your board connects them"* — reported live building the divider. Fix: add `SYMMETRIC_KINDS = {resistor, capacitor, inductor}`; resolve symmetric parts' leg→terminal by propagation from the fixed anchors (ports, polarized legs, DIP/TO-92 pins) so either orientation passes; polarized parts unchanged. Pure `core/breadboard.ts` + tests, **no `core/signal.ts`**. See `docs/specs/symmetric-passive-orientation.md`. | ARB-3, ARB-5b | **DONE** |
