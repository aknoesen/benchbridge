...
describe('SCH-11 sim-result equivalence across the example library', () => {
  it('every example reads the same CH1/CH2 waveforms as the committed baseline', async () => {
    ...
    // (The 'ina125-amp' example — the one deliberate baseline exception, shipped broken with its
    // RG pins shorted by the old reference stubs — was removed from the library entirely, so the
    // harness covers every remaining example against its unchanged baseline.)
    for (const ex of EXAMPLES) {
      const b = base[ex.id]
